...
interface OrgAdminLayoutProps {
  ...
  orgName?: string;
  /** Set to true when a system admin has entered an org's management context */
...
export function OrgAdminLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      {/* System-admin-in-org context banner */}
      {isNestedInSystem && (
        <div className="bg-amber-50 border-b border-amber-200 px-4 py-1.5 text-xs text-amber-800 flex items-center gap-2">
          <span className="font-medium">Viewing as System Admin</span>
          <span className="text-amber-500">·</span>
          <span>Managing: {displayOrgName}</span>
          <a href="/system/organizations" className="ml-auto text-amber-700 underline underline-offset-2 hover:text-amber-900">
